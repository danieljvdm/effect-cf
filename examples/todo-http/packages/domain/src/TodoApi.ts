...
export const TodosGroup = HttpApiGroup.make("Todos")
  .add(
    HttpApiEndpoint.get("listTodos", "/todos", { success: TodoList, error: DatabaseErrorResponse }),
  )
  .add(
  ...
  .add(
    HttpApiEndpoint.delete("clearCompleted", "/todos/completed", {
      success: TodoStats,
      error: DatabaseErrorResponse,
    }),
  )
  ...
  .add(
    HttpApiEndpoint.get("stats", "/stats", { success: TodoStats, error: DatabaseErrorResponse }),
  );
