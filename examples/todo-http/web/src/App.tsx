import type { Todo, TodoStats } from "@effect-cf/todo-http-domain";
import { Effect } from "effect";
import { useCallback, useEffect, useMemo, useState } from "react";
import { runClient, TodoApiClient } from "./clients";
...
export default function App() {
  ...
  const [draft, setDraft] = useState("Ship a focused typed HTTP API todo demo");
  const [status, setStatus] = useState("Booting typed HTTP API client…");
  ...
  const run = async (label: string, effect: Effect.Effect<void, unknown, TodoApiClient>) => {
    setIsBusy(true);
    setStatus(label);
    try {
  ...
  const addTodo = () =>
    run(
      "Creating todo through HttpApiClient POST /todos…",
  ...
  const toggleTodo = (todo: Todo) =>
    run("Updating todo through typed HTTP API…", updateTodo(todo).pipe(Effect.asVoid));
  const deleteTodo = (todo: Todo) =>
    run("Deleting todo through typed HTTP API…", removeTodo(todo).pipe(Effect.asVoid));
  const seedTodos = () =>
    run(
      "Seeding todos through typed HTTP API…",
      Effect.forEach(initialTitles, (title) => createTodo(title), { discard: true }),
    );
  const clearCompleted = () =>
    run(
      "Clearing completed todos through typed HTTP API…",
  ...
  return (
    <main className="shell">
      <section className="hero">
        <p className="eyebrow">Effect Cloudflare · D1 · HttpApi · HttpApiClient</p>
        <h1>A todo ledger over typed HTTP</h1>
        <p className="lede">
          This focused demo uses only Effect HttpApi routes in the Worker and a browser
          HttpApiClient. D1 persistence stays behind the Worker runtime boundary; the browser never
          imports RPC transport code.
        </p>
      </section>
