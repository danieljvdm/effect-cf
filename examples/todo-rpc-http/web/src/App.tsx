import type { Todo, TodoStats } from "@effect-cf/todo-rpc-http-domain";
...
export default function App() {
  ...
  const [draft, setDraft] = useState("Ship a focused Effect RPC over HTTP todo demo");
  const [status, setStatus] = useState("Booting Effect RPC over HTTP client…");
  ...
  const load = useCallback(async () => {
    ...
    setTodos(list.todos);
    setStats(nextStats);
    setStatus(`Synced ${list.todos.length} todo(s) via Effect RPC over HTTP`);
  }, []);
  useEffect(() => {
  ...
  const addTodo = () =>
    run(
      "Creating todo through RpcClient HTTP CreateTodo…",
  ...
  const toggleTodo = (todo: Todo) =>
    run("Updating todo through Effect RPC over HTTP…", updateTodo(todo).pipe(Effect.asVoid));
  const deleteTodo = (todo: Todo) =>
    run("Deleting todo through Effect RPC over HTTP…", removeTodo(todo).pipe(Effect.asVoid));
  const seedTodos = () =>
    run(
      "Seeding todos through Effect RPC over HTTP…",
      Effect.forEach(initialTitles, (title) => createTodo(title), { discard: true }),
    );
  const clearCompleted = () =>
    run(
      "Clearing completed todos through Effect RPC over HTTP…",
  ...
  return (
    <main className="shell">
      <section className="hero">
        <p className="eyebrow">Effect Cloudflare · D1 · RpcGroup · HTTP RPC</p>
        <h1>Todos by request-response RPC</h1>
        <p className="lede">
          This focused demo removes typed HTTP routes entirely. The Worker exposes one Effect RPC
          HTTP endpoint at /api/rpc, and the browser uses RpcClient.layerProtocolHttp for every todo
          operation.
        </p>
      </section>
